...
public class FuncionarioController {
    ...
    public ResponseEntity<?> listaId(@PathVariable(value = "id") Long id){
        Funcionario listarid = Repository.findById(id).orElse(null);
        return listarid == null
                ? ResponseEntity.badRequest().body(" <<ERRO>>: valor nao encontrado.")
                : ResponseEntity.ok(listarid);
    }
    @GetMapping("/lista/ativo/{ativo}")
    public ResponseEntity<List<Funcionario>> listaAtivo(@PathVariable boolean ativo) {
        List<Funcionario> listarAtivo = Repository.findByAtivo(ativo);
        return ResponseEntity.ok(listarAtivo);
    }

    @PostMapping("/cadastrar")
    public ResponseEntity<?> cadastrar(@RequestBody Funcionario cadastro){
        try{
    ...
    public ResponseEntity<?> atualizar( @PathVariable Long id, @RequestBody Funcionario atualizarId) {
        try {
